...
def monthly_report_email(
    *, tenant: dict[str, Any], year: int, month: int
    ...
    body_html = (
        ...
        + f'<p style="margin:0 0 14px 0;">{_greeting(tenant)}</p>'
        + f'<p style="margin:0 0 14px 0;">Your monthly SmartTap report for <strong>{_escape(business)}</strong> is attached as a PDF. Inside you\'ll find new customers, taps, stamps and rewards for {_escape(period_label)}, with comparisons to the previous month — plus your Google reviews and average rating.</p>'
    ...
    text = (
        ...
        "Hi there,\n\n"
        f"Your monthly report for {business} is attached as a PDF — new customers, taps, stamps and rewards for {period_label}, plus your Google reviews and average rating.\n\n"
        f"Open the dashboard: {SITE_URL}/dashboard"
    )
    return RenderedEmail(
