 ...
 *
 * Kept inline (not an <img>) so it gets the brand colour from currentColor
 * and never adds a network request. The wave arcs imply NFC tap radius
 * without literal radio glyphs — quieter than the official tap icon, and
 * lets the mark scale down to favicon size without losing identity.
 *
...
export function BrandLogo({
  ...
  return (
    <span
      className={cn("inline-flex items-center gap-2 text-green-900", className)}
    >
      <svg
        width={size}
        height={size}
      ...
      >
        <title>{title}</title>
        {/* Rounded square plate — solid green */}
        <rect width="40" height="40" rx="9" fill="currentColor" />
        {/* "ST" monogram in amber, optical-centered */}
        <path
